...
class RESTArtistTests(TestCase):
    ...
    def test_get_artist(self):
        response = self.client.get('/api/artist/?name=The Beatles')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_add_artist(self):
        data = {
            "name": "Squeezie",
            "song": ["Time time", "Top 1", "Ce petit coté Kawai"]
        }
        response = self.client.post('/api/artist/', data, format='json')
    ...
    def test_add_song_with_artist_created(self):
        ...
        response = self.client.patch('/api/artist/1/add_song/', data, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_remove_song(self):
        ...
        response = self.client.patch("/api/artist/1/remove_song/", data, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
